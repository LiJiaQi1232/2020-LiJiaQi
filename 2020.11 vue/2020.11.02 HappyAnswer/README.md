...
<!-- 选项 -->
<ul>
  <li class="item_list" v-for="(item) in questions[itemNum-1].topic_answer" :key="item.topic_answer_id">
    <span class="option_style">A</span>
...
```html
<span class="option_style">{{chooseType(index)}}</span>(忽略)
